    ...
    if (!state.win) {
        if (!block.filled) {
            for (let i = 0; i < block.n; i++) {
                block.elements[0][i] = { x: Constants.MAX_WIDTH / Constants.PSEUDO_PIXEL / 4,
                                         y: 70 - i * 5, width : 20 - i * 3
                };
            }
        ...
        if (start) {
            ...
            for (let i = 0; i < 3; i++) {
                    if (block.elements[i].length != 0 &&
                        distance(block.elements[i][block.elements[i].length - 1].x,
                                 block.elements[i][block.elements[i].length - 1].y,
                                 start.event.pageX / Constants.PSEUDO_PIXEL,
                                 start.event.pageY / Constants.PSEUDO_PIXEL) < 5)
                    {
                        block.elx = i;
        ...
        if (block.ely != -1) {
            ...
            if (end) {
                ...
                for (let i = 0; i < 3; i++) {
                    if (block.elements[block.elx][block.ely].x - Constants.MAX_WIDTH / Constants.PSEUDO_PIXEL / 4 * (i+1) < 8 &&
                        block.elements[block.elx][block.ely].x - Constants.MAX_WIDTH / Constants.PSEUDO_PIXEL / 4 * (i+1) > -8)
                    {
                        if ((block.elements[i].length != 0 && block.elements[block.elx][block.ely].width < block.elements[i][block.elements[i].length - 1].width)
